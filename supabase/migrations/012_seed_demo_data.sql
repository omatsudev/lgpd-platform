-- ============================================================
-- SEED: Dados de demonstração realistas para todas as telas
-- Detecta o primeiro usuário/empresa e popula tudo
-- ============================================================
...
DECLARE
  ...
  -- IDs de registros que serão referenciados
...
BEGIN
  ...
  IF v_company_id IS NULL THEN
    INSERT INTO public.companies (name, slug, sector, dpo_name, dpo_email, dpo_phone, owner_id)
    VALUES ('Nexus Tecnologia Ltda.', 'nexus-tecnologia', 'Tecnologia',
            'Ana Carolina Mendes', 'dpo@nexustec.com.br', '(11) 99876-5432', v_user_id)
  ...
  -- ── 2. Inventário de dados (5 processos) ─────────────────
  inv1 := gen_random_uuid();
  inv2 := gen_random_uuid();
  inv3 := gen_random_uuid();
  inv4 := gen_random_uuid();
  ...
  (inv2, v_company_id, 'dados sensiveis',
   ...
   'contínuo', true, 'Contabilidade externa (ABC Contadores Ltda.), Operadora de saúde',
   'Cumprimento de obrigações trabalhistas, previdenciárias e pagamento de salários.',
   'Cumprimento de obrigação legal',
   'direto', 'funcionario', 'hibrido', 'Sistema Totvs + servidor local',
  ...
  (inv3, v_company_id, 'dados consentimento',
   ...
   'contínuo', true, 'Plataforma RD Station, Meta Ads, Google Ads',
   'Envio de newsletters, ofertas personalizadas e anúncios segmentados.',
   'Consentimento do titular',
   'direto', 'lead', 'nuvem', 'RD Station CRM + Google Analytics',
  ...
  (inv5, v_company_id, 'imagem video voz',
   ...
   'Legítimo interesse',
   'direto', 'funcionario', 'fisico', 'DVR local + sistema de controle de acesso CISA',
  ...
  -- ── 3. Fornecedores (4 registros) ─────────────────────────
  sup1 := gen_random_uuid();
  sup2 := gen_random_uuid();
  sup3 := gen_random_uuid();
  sup4 := gen_random_uuid();
  ...
  -- ── 4. Documentos (4 registros) ───────────────────────────
  doc1 := gen_random_uuid();
  doc2 := gen_random_uuid();
  doc3 := gen_random_uuid();
  doc4 := gen_random_uuid();
  ...
  (doc1, v_company_id,
   ...
   '2.1', 'published', 'Ana Carolina Mendes (DPO)',
  ...
  (doc4, v_company_id,
   'Relatório de Impacto à Proteção de Dados (RIPD) — RH', 'dpia',
   'RIPD elaborado para o processo de Folha de Pagamento que trata dados sensíveis de saúde.',
   '1.2', 'under_review', 'Ana Carolina Mendes (DPO)',
   null,
   (CURRENT_DATE + INTERVAL '15 days')::date,
   null,
   v_user_id);

  -- ── 5. Riscos (5 registros) ───────────────────────────────
  risk1 := gen_random_uuid();
  risk2 := gen_random_uuid();
  risk3 := gen_random_uuid();
  risk4 := gen_random_uuid();
  risk5 := gen_random_uuid();
  ...
  (risk2, v_company_id,
   ...
   'A ABC Contadores processa dados de saúde de colaboradores sem Acordo de Processamento de Dados assinado.',
   'legal', 'supplier',
   3, 5, 3, 5,
   'mitigate',
   'Formalizar DPA com a ABC Contadores até 30 dias. Incluir cláusulas de sub-operador e auditoria.',
   'Jurídico / DPO',
   (CURRENT_DATE + INTERVAL '30 days')::date,
  ...
  (risk4, v_company_id,
   ...
   'Dados de clientes trafegam para servidores do Zendesk nos EUA sem cláusulas contratuais padrão formalizadas.',
   'legal', 'supplier',
   2, 4, 2, 3,
   'mitigate',
   'Verificar Standard Contractual Clauses (SCCs) no contrato Zendesk. Consultar ANPD sobre transferências para EUA.',
   'Jurídico',
  ...
  (risk5, v_company_id,
   ...
   'Parte dos colaboradores ainda não concluiu treinamento obrigatório, aumentando risco de erros e vazamentos.',
  ...
  (inc2, v_company_id,
   ...
   'RH / Jurídico / TI',
   true, false, v_user_id);

  -- ── 7. Solicitações de titulares (5 registros) ────────────
  req1 := gen_random_uuid();
  req2 := gen_random_uuid();
  req3 := gen_random_uuid();
  req4 := gen_random_uuid();
  req5 := gen_random_uuid();

  INSERT INTO public.data_subject_requests
    (id, company_id, type, name, email, cpf, description, status, response, response_deadline)
  VALUES
  (req1, v_company_id,
   'access', 'Carlos Eduardo Martins', 'carlos.martins@email.com', '123.456.789-00',
   'Gostaria de saber quais dados pessoais meus a empresa possui e como são utilizados.',
   'completed',
   'Prezado Carlos, seguem seus dados em nossa base: nome, e-mail, telefone, histórico de compras. Tratamos com base em execução de contrato. Atenciosamente, DPO.',
   (CURRENT_DATE - INTERVAL '5 days')::date),

  (req2, v_company_id,
   'deletion', 'Mariana Costa Oliveira', 'mariana.costa@gmail.com', '987.654.321-00',
  ...
  (req3, v_company_id,
   'correction', 'Roberto Alves da Silva', 'roberto.alves@empresa.com', '456.789.123-00',
   'Meu endereço está incorreto no cadastro. O correto é Rua das Flores, 123, Apt 45, São Paulo/SP.',
   'pending', null,
   (CURRENT_DATE + INTERVAL '3 days')::date),

  (req4, v_company_id,
   'portability', 'Fernanda Lima', 'fernanda.lima@outlook.com', null,
   'Solicito a portabilidade dos meus dados de perfil para outra plataforma, no formato JSON ou CSV.',
   'pending', null,
   (CURRENT_DATE + INTERVAL '10 days')::date),

  (req5, v_company_id,
   'objection', 'Paulo Henrique Rocha', 'paulo.rocha@yahoo.com', null,
   'Me oponho ao uso dos meus dados para campanhas de marketing. Não autorizo mais o envio de comunicações.',
   'completed',
   'Opt-out realizado com sucesso. Seus dados foram removidos de todas as listas de marketing. O descadastramento foi registrado.',
   (CURRENT_DATE - INTERVAL '20 days')::date);

  -- ── 8. Treinamentos + Colaboradores ───────────────────────
  tr1 := gen_random_uuid();
  tr2 := gen_random_uuid();

  INSERT INTO public.trainings
    (id, company_id, title, description, created_by)
  VALUES
  (tr1, v_company_id,
  ...
  VALUES
  (tr1, 'Ana Paula Ferreira',   'ana.ferreira@nexustec.com.br',  '(11)99111-1111', gen_random_uuid()::text, 'completed', 100, NOW() - INTERVAL '5 days'),
  (tr1, 'Bruno Carvalho',       'bruno.carvalho@nexustec.com.br','(11)99222-2222', gen_random_uuid()::text, 'completed', 100, NOW() - INTERVAL '3 days'),
  (tr1, 'Camila Santos',        'camila.santos@nexustec.com.br', '(11)99333-3333', gen_random_uuid()::text, 'completed', 100, NOW() - INTERVAL '1 day'),
  (tr1, 'Diego Mendonça',       'diego.mendonca@nexustec.com.br','(11)99444-4444', gen_random_uuid()::text, 'in_progress', 60, null),
  (tr1, 'Eduarda Pinto',        'eduarda.pinto@nexustec.com.br', '(11)99555-5555', gen_random_uuid()::text, 'in_progress', 30, null),
  (tr1, 'Felipe Torres',        'felipe.torres@nexustec.com.br', '(11)99666-6666', gen_random_uuid()::text, 'not_started', 0, null),
  (tr1, 'Gabriela Rodrigues',   'gabriela.rodrigues@nexustec.com.br','(11)99777-7777', gen_random_uuid()::text, 'not_started', 0, null),
  (tr2, 'Ana Paula Ferreira',   'ana.ferreira@nexustec.com.br',  '(11)99111-1111', gen_random_uuid()::text, 'completed', 100, NOW() - INTERVAL '2 days'),
  (tr2, 'Henrique Bastos',      'henrique.bastos@nexustec.com.br','(11)99888-8888', gen_random_uuid()::text, 'completed', 100, NOW() - INTERVAL '1 day'),
  (tr2, 'Isabela Nunes',        'isabela.nunes@nexustec.com.br', '(11)99999-9999', gen_random_uuid()::text, 'in_progress', 75, null),
  (tr2, 'João Victor Lima',     'joao.lima@nexustec.com.br',     '(11)98111-0000', gen_random_uuid()::text, 'not_started', 0, null);

  -- ── 9. Consentimentos + Finalidades ──────────────────────
  cp1 := gen_random_uuid();
  cp2 := gen_random_uuid();
  cp3 := gen_random_uuid();
  ...
  -- ── 10. Denúncias (2 registros) ───────────────────────────
  comp1 := gen_random_uuid();
  comp2 := gen_random_uuid();
  ...
  (comp1, v_company_id,
   ...
   'under_review'),
  (comp2, v_company_id,
   false, 'Colaboradora do RH', 'colaboradora.rh@interno.com', 'privacy_violation',
  ...
  -- ── 11. Scans de Cookies (2 registros) ───────────────────
  ...
  BEGIN
    ...
    VALUES
    (scan1, v_company_id, v_company_id,
     'https://nexustec.com.br', 'nexustec.com.br', 'nexustec.com.br', 'completed', 72,
     true, true, 'https://nexustec.com.br/privacidade',
     '[{"name":"_ga","domain":".nexustec.com.br","type":"analytics","duration":"2 anos"},{"name":"_fbp","domain":".nexustec.com.br","type":"marketing","duration":"90 dias"},{"name":"session_id","domain":"nexustec.com.br","type":"essential","duration":"sessão"}]',
     '[{"name":"Google Analytics","category":"analytics"},{"name":"Facebook Pixel","category":"marketing"},{"name":"Hotjar","category":"analytics"}]',
     '[{"severity":"high","description":"Cookies de marketing carregados antes do consentimento"},{"severity":"medium","description":"Banner sem opção de rejeitar todos"}]',
     '[{"priority":"high","description":"Bloquear cookies de marketing até consentimento explícito"},{"priority":"medium","description":"Adicionar botão Rejeitar Todos no banner"}]',
     v_user_id),
    (scan2, v_company_id, v_company_id,
     'https://app.nexustec.com.br', 'app.nexustec.com.br', 'app.nexustec.com.br', 'completed', 88,
     true, true, 'https://nexustec.com.br/privacidade',
     '[{"name":"session_token","domain":"app.nexustec.com.br","type":"essential","duration":"24h"},{"name":"_ga","domain":".nexustec.com.br","type":"analytics","duration":"2 anos"}]',
     '[{"name":"Google Analytics","category":"analytics"}]',
     '[{"severity":"low","description":"Cookie _ga de analytics ativo sem aviso específico"}]',
     '[{"priority":"low","description":"Incluir nota sobre analytics no aviso de privacidade"}]',
     v_user_id);
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'site_scans seed ignorado (colunas incompatíveis): %', SQLERRM;
  END;

  -- ── 12. Checklist — 20 itens concluídos ──────────────────
  ...
  RAISE NOTICE 'Seed de demo concluído para company_id: %', v_company_id;
