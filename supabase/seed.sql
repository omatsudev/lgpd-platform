-- ============================================================
-- SEED: Dados de demonstração — apenas sandbox/desenvolvimento
-- NÃO aplicar em produção.
-- Roda automaticamente em: supabase db reset (local)
-- Para staging: supabase db query --local -f supabase/seed.sql
-- ============================================================
...
BEGIN
  ...
  -- Pular se já existirem dados de inventário
  IF EXISTS (SELECT 1 FROM public.data_inventory WHERE company_id = v_company_id) THEN
    RAISE NOTICE 'Dados já existem para company_id: %. Seed ignorado.', v_company_id;
    RETURN;
  END IF;
  ...
  (inv1, v_company_id, 'personal identification',
   ...
   '{"collection":{"active":true,"controller":true,"processor":false},"use":{"active":true,"controller":true,"processor":true},"storage":{"active":true,"controller":true,"processor":false},"sharing":{"active":false,"controller":false,"processor":false},"retention":{"active":true,"controller":true,"processor":false},"disposal":{"active":true,"controller":true,"processor":false}}'::jsonb,
   '["personal_identification","government_data","financial_data"]'::jsonb,
   ...
   'direto', 'cliente', 'cloud', 'AWS S3 — região sa-east-1',
  ...
  -- 2. Folha de Pagamento e RH
  (inv2, v_company_id, 'sensitive data',
   'Folha de Pagamento e RH', 'Recursos Humanos',
   'Processamento de dados de colaboradores para fins trabalhistas e previdenciários.',
   '{"collection":{"active":true,"controller":true,"processor":false},"use":{"active":true,"controller":true,"processor":true},"storage":{"active":true,"controller":true,"processor":false},"sharing":{"active":true,"controller":true,"processor":false},"retention":{"active":true,"controller":true,"processor":false},"disposal":{"active":true,"controller":true,"processor":false}}'::jsonb,
   '["personal_identification","government_data","financial_data","sensitive_data"]'::jsonb,
   ...
   'direto', 'funcionario', 'hybrid', 'Sistema ERP + servidor local',
  ...
  (inv3, v_company_id, 'consent data',
   'Marketing e Comunicações', 'Marketing',
   'Envio de comunicações comerciais e campanhas para leads e clientes.',
   '{"collection":{"active":true,"controller":true,"processor":false},"use":{"active":true,"controller":true,"processor":true},"storage":{"active":true,"controller":false,"processor":false},"sharing":{"active":true,"controller":false,"processor":true},"retention":{"active":true,"controller":false,"processor":false},"disposal":{"active":true,"controller":true,"processor":false}}'::jsonb,
   '["personal_identification","electronic_identification","consent_data"]'::jsonb,
   ...
   'direto', 'lead', 'cloud', 'CRM + Google Analytics',
  ...
  (inv4, v_company_id, 'electronic data',
   'Suporte e Atendimento ao Cliente', 'Customer Success',
   'Registro e resolução de chamados de suporte técnico e atendimento.',
   '{"collection":{"active":true,"controller":true,"processor":false},"use":{"active":true,"controller":true,"processor":false},"storage":{"active":true,"controller":true,"processor":false},"sharing":{"active":false,"controller":false,"processor":false},"retention":{"active":true,"controller":true,"processor":false},"disposal":{"active":true,"controller":true,"processor":false}}'::jsonb,
   '["personal_identification","electronic_identification"]'::jsonb,
   'Nome, e-mail, histórico de chamados, logs de acesso ao sistema.',
   'contínuo', false, null,
   'Resolução de problemas técnicos, melhoria de produto e NPS.',
   'Execução de contrato',
   'direto', 'cliente', 'cloud', 'Zendesk Cloud',
   '3 anos após encerramento do contrato',
   'Customer Success', 'TLS em trânsito, dados mascarados em logs públicos.',
   'nao', 'low', 'complete', v_user_id),

  -- 5. Monitoramento de Acesso Físico
  (inv5, v_company_id, 'image video voice',
   ...
   '{"collection":{"active":true,"controller":true,"processor":false},"use":{"active":true,"controller":false,"processor":false},"storage":{"active":true,"controller":true,"processor":false},"sharing":{"active":false,"controller":false,"processor":false},"retention":{"active":true,"controller":true,"processor":false},"disposal":{"active":true,"controller":true,"processor":false}}'::jsonb,
   '["personal_identification","image_video_voice"]'::jsonb,
   'Nome, RG, foto, horário de entrada/saída, imagens de câmeras.',
   'contínuo', false, null,
   'Segurança patrimonial e controle de acesso às instalações.',
   'Legítimo interesse',
   'direto', 'funcionario', 'paper', 'DVR local + sistema de controle de acesso',
  ...
  (inc1, v_company_id,
   ...
   '["electronic_identification","personal_identification"]', 1,
  ...
  RAISE NOTICE 'Seed concluído para company_id: %', v_company_id;
END $$;
