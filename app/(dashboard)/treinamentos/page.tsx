...
export default async function TreinamentosPage({ searchParams }: { searchParams: Promise<{ q?: string; aba?: string }> }) {
  const { companyId, supabase } = await getUserCompany()
  const { q, aba = 'treinamentos' } = await searchParams
  ...
  const { data: treinamentosColabs } = companyId
    ? await supabase
        .from('trainings')
  ...
  const colaboradoresMap = new Map<string, { name: string; email: string | null; whatsapp: string | null; total: number; completed: number }>()
  for (const t of (treinamentosColabs ?? [])) {
    for (const c of ((t as any).training_employees ?? [])) {
      const key = c.employee_email ?? c.employee_name
      if (!colaboradoresMap.has(key)) {
        colaboradoresMap.set(key, { name: c.employee_name, email: c.employee_email, whatsapp: c.employee_whatsapp, total: 0, completed: 0 })
      }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-1 border-b border-gray-200">
        {[
          { key: 'treinamentos', label: `Treinamentos (${treinamentos.length})` },
          { key: 'colaboradores', label: `Colaboradores (${colaboradoresMap.size})`, icon: Users },
        ].map(tab => (
          <Link
            key={tab.key}
            href={`/treinamentos?aba=${tab.key}${q ? `&q=${q}` : ''}`}
      ...
      <SearchInput defaultValue={q ?? ''} placeholder={aba === 'colaboradores' ? 'Buscar por nome ou email...' : 'Buscar por título ou descrição...'} />
