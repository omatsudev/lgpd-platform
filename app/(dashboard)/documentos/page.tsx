...
export default async function DocumentosPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const hoje = new Date()
  const expirados = itens.filter((d: any) => d.expiration_date && new Date(d.expiration_date) < hoje).length
                    ...
                    {itens.map((item: any) => {
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-gray-500 text-xs font-mono">v{item.version}</td>
                          <td className="py-3 px-4">
                            <Badge variant={statusVariant[item.status] ?? 'secondary'}>
                              {statusLabel[item.status] ?? item.status}
                            </Badge>
                          </td>
                          <td className="py-3 px-4 text-gray-500 text-xs">{item.review_date ? formatDate(item.review_date) : '—'}</td>
