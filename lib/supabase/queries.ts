...
export const getUserCompany = cache(async function getUserCompany() {
  ...
  if (!user) return { user: null, company: null, companyId: null, role: null, companies: [], supabase }
  ...
  const targetUc =
    (selectedId && ucRows.find(uc => uc.company_id === selectedId)) ||
    ucRows[0]
  ...
  const { data: allCompanies } = await supabase
    .from('companies')
    .select('id, name')
    .in('id', ucRows.map(uc => uc.company_id))
