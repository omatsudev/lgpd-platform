...
const TECH_SIGNATURES: Array<{
  ...
  {
    pattern: /google-analytics\.com|gtag|ga\s*\(|_ga\b/i,
    tech: {
      name: 'Google Analytics',
      category: 'analytics',
      risk: 'high',
      description: 'Rastreamento de comportamento via Google',
    },
  },
  {
    pattern: /googletagmanager\.com|GTM-/i,
    tech: {
  ...
  {
    pattern: /facebook\.net|fbq\(|fb-pixel|_fbp\b/i,
    tech: {
      name: 'Facebook Pixel',
      category: 'advertising',
      risk: 'high',
      description: 'Pixel de rastreamento do Meta/Facebook',
    },
  },
  {
    pattern: /connect\.facebook\.net/i,
    tech: {
      name: 'Facebook SDK',
      category: 'social',
      risk: 'high',
      description: 'SDK social do Facebook',
    },
  },
  {
    pattern: /hotjar\.com|hotjar/i,
    tech: {
      name: 'Hotjar',
      category: 'analytics',
      risk: 'medium',
      description: 'Gravação de sessão e mapas de calor',
    },
  },
  {
    ...
    tech: {
      ...
      category: 'analytics',
      risk: 'medium',
      description: 'Ferramenta de analytics da Microsoft',
    },
  },
  {
    pattern: /linkedin\.com\/insight|_linkedin_/i,
    tech: {
      name: 'LinkedIn Insight Tag',
      category: 'advertising',
      risk: 'high',
      description: 'Tag de rastreamento do LinkedIn',
    },
  },
  {
    pattern: /tiktok\.com\/i18n\/pixel|ttq\./i,
    tech: {
      name: 'TikTok Pixel',
      category: 'advertising',
      risk: 'high',
      description: 'Pixel de rastreamento do TikTok',
    },
  },
  {
    pattern: /rd\.station|rdstation/i,
    tech: {
      name: 'RD Station',
      category: 'analytics',
      risk: 'medium',
      description: 'Plataforma de automação de marketing brasileira',
    },
  },
  {
    pattern: /hubspot\.com\/|_hstc\b/i,
    tech: {
      name: 'HubSpot',
      category: 'analytics',
      risk: 'medium',
  ...
  {
    pattern: /intercom\.io|intercomSettings/i,
    tech: {
      name: 'Intercom',
      category: 'chat',
      risk: 'medium',
  ...
  {
    pattern: /crisp\.chat|CRISP_WEBSITE_ID/i,
    tech: { name: 'Crisp Chat', category: 'chat', risk: 'low', description: 'Widget de chat' },
  },
  {
    pattern: /wp-content|wp-includes|wordpress/i,
    tech: { name: 'WordPress', category: 'cms', risk: 'low', description: 'CMS WordPress' },
  },
  {
    ...
    tech: {
      ...
      category: 'cms',
      risk: 'low',
      description: 'Plataforma de e-commerce Shopify',
    },
  },
  {
    pattern: /doubleclick\.net|googlesyndication/i,
    tech: {
      name: 'Google Ads',
      category: 'advertising',
      risk: 'high',
      description: 'Rede de publicidade do Google',
    },
  },
  {
    pattern: /mixpanel\.com|mixpanel\.track/i,
    tech: {
      name: 'Mixpanel',
      category: 'analytics',
      risk: 'medium',
  ...
  },
  {
    ...
    tech: {
      ...
      category: 'analytics',
      risk: 'medium',
      description: 'Plataforma de dados de clientes',
    },
  },
  {
    pattern: /sentry\.io|Sentry\.init/i,
    tech: { name: 'Sentry', category: 'other', risk: 'low', description: 'Monitoramento de erros' },
  },
]
...
function parseCookieHeader(header: string): Partial<Cookie> {
  ...
  return {
    name: (rawName ?? '').trim(),
    value: rawValue
      ? rawValue.trim().substring(0, 20) + (rawValue.length > 20 ? '...' : '')
      : undefined,
    path: attrMap['path'],
...
export async function scanSite(url: string): Promise<ScanResult> {
  ...
  const hasMarketingCookies =
    cookies.some((c) => c.category === 'marketing') ||
    technologies.some((t) => t.category === 'advertising')
  const hasAnalytics =
    cookies.some((c) => c.category === 'analytical') ||
  ...
  if (!privacyPolicy.exists) {
    issues.push({
      ...
      description:
        'Nenhuma Política de Privacidade foi encontrada no site. Obrigatório pela LGPD (Art. 9).',
    })
    recommendations.push({
      title: 'Publicar Política de Privacidade',
      description:
        'Crie e publique uma Política de Privacidade clara e acessível com link no rodapé.',
    })
  }
  ...
  if ((hasMarketingCookies || hasAnalytics) && !has_cookie_banner) {
    issues.push({
      ...
      description: 'O site usa cookies de rastreamento mas não apresenta banner de consentimento.',
    })
    recommendations.push({
      title: 'Implementar banner de consentimento',
      description:
        'Adicione um banner informando sobre o uso de cookies e coletando consentimento antes de ativá-los.',
    })
  }
  ...
  if (hasInternationalTransfer) {
    issues.push({
      ...
      description:
        'Scripts de terceiros (Google, Meta, etc.) transferem dados para servidores fora do Brasil. Exige base legal adequada (LGPD Art. 33).',
    })
    recommendations.push({
      title: 'Documentar transferências internacionais',
      description:
        'Identifique todos os destinos, inclua na Política de Privacidade e garanta mecanismo legal (SCC ou consentimento).',
    })
  }
  ...
  if (advertisingTools.length > 0) {
    issues.push({
      level: 'high',
      title: `${advertisingTools.length} ferramenta(s) de publicidade detectada(s)`,
      description: `${advertisingTools.map((t) => t.name).join(', ')} coletam dados para fins de publicidade. Requer consentimento explícito.`,
    })
  }
  ...
  if (hasInsecureCookies && cookies.some((c) => !c.secure && c.category !== 'necessary')) {
    issues.push({
      ...
      description:
        'Alguns cookies são transmitidos por HTTP não seguro, expondo dados dos titulares.',
    })
    recommendations.push({
      title: 'Adicionar flag Secure aos cookies',
      description:
        'Configure todos os cookies com Secure=true para forçar transmissão apenas via HTTPS.',
    })
  }
  ...
  if (hasMissingSameSite) {
    issues.push({
      level: 'low',
      title: 'Cookies sem atributo SameSite',
      description:
        'Cookies sem SameSite podem ser enviados em requisições cross-site, aumentando risco de CSRF.',
    })
    recommendations.push({
      title: 'Configurar SameSite nos cookies',
      description: 'Defina SameSite=Strict ou SameSite=Lax em todos os cookies.',
    })
  }
  ...
  if (cookies.length === 0 && technologies.length === 0) {
    recommendations.push({
      ...
      description:
        'Execute o scan periodicamente para detectar novos scripts adicionados por terceiros ou atualizações de CMS.',
    })
  }
