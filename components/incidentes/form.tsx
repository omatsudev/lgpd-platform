...
const SEVERITIES = [
  ...
  { value: 'critical', label: 'Crítica', desc: 'Dados sensíveis em escala, risco imediato aos titulares' },
]
...
type FormState = Omit<IncidenteData, 'company_id' | 'id'>
...
export function IncidenteForm({ companyId, id, initialData }: IncidentFormProps) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await salvarIncidente({ ...data, company_id: companyId, id })
    } catch {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (formData: FormData) => {
    startDeleting(async () => { await deletarIncidente(formData) })
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
              ...
              {SEVERITIES.map(s => (
                <button
                  ...
                  className={`p-3 rounded-lg border-2 text-left transition-all ${
                    data.severity === s.value ? severityColors[s.value] : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
                  }`}
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input id="occurrence_date" type="date" value={data.occurrence_date} onChange={e => update('occurrence_date', e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="discovery_date">Data de descoberta *</Label>
              <Input id="discovery_date" type="date" value={data.discovery_date} onChange={e => update('discovery_date', e.target.value)} />
              {errors.discovery_date && <p className="text-xs text-red-500">{errors.discovery_date}</p>}
            </div>
          </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Textarea id="affected_data" value={data.affected_data} onChange={e => update('affected_data', e.target.value)} placeholder="Descreva quais dados foram expostos ou comprometidos..." rows={3} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="affected_subjects_count">Titulares afetados</Label>
              <Input id="affected_subjects_count" value={data.affected_subjects_count} onChange={e => update('affected_subjects_count', e.target.value)} placeholder="Ex: Aproximadamente 500, Desconhecido" />
              <Label htmlFor="root_cause" className="mt-3 block">Causa raiz</Label>
              <Textarea id="root_cause" value={data.root_cause} onChange={e => update('root_cause', e.target.value)} placeholder="Qual foi a origem do incidente?" rows={2} />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Textarea id="immediate_measures" value={data.immediate_measures} onChange={e => update('immediate_measures', e.target.value)} placeholder="O que foi feito imediatamente após descoberta? (bloqueio, isolamento, backup...)" rows={3} />
          </div>
          <div className="space-y-1.5">
      ...
      <Card>
        <CardHeader className="pb-2">
          <SectionHeader icon={Bell} title="Notificações Regulatórias" subtitle="LGPD Art. 48: comunicação à ANPD e titulares" />
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="p-4 rounded-lg border border-gray-200 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {data.notified_anpd && <Badge variant="success"><CheckCircle2 className="h-3 w-3 mr-1" />Notificado</Badge>}
            </div>
            <CheckBox checked={data.notified_anpd} onChange={v => update('notified_anpd', v)} label="A ANPD foi notificada sobre este incidente" />
            {data.notified_anpd && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                <div className="space-y-1.5">
                  <Label htmlFor="anpd_notification_date">Data da notificação</Label>
                  <Input id="anpd_notification_date" type="date" value={data.anpd_notification_date} onChange={e => update('anpd_notification_date', e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="anpd_protocol">Protocolo ANPD</Label>
                  <Input id="anpd_protocol" value={data.anpd_protocol} onChange={e => update('anpd_protocol', e.target.value)} placeholder="Número do protocolo" />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="p-4 rounded-lg border border-gray-200 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {data.notified_subjects && <Badge variant="success"><CheckCircle2 className="h-3 w-3 mr-1" />Notificados</Badge>}
            </div>
            <CheckBox checked={data.notified_subjects} onChange={v => update('notified_subjects', v)} label="Os titulares afetados foram comunicados" />
            {data.notified_subjects && (
              <div className="pt-1">
                <Label htmlFor="subjects_notification_date">Data da comunicação aos titulares</Label>
                <Input id="subjects_notification_date" type="date" value={data.subjects_notification_date} onChange={e => update('subjects_notification_date', e.target.value)} className="mt-1.5" />
              </div>
            )}
          </div>
        </CardContent>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3">
        ...
        <div className="flex gap-3 sm:ml-auto">
          <Button variant="outline" onClick={() => router.push('/incidents')} disabled={saving}>Cancelar</Button>
